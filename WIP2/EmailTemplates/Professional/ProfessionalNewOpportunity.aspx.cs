        ...
        protected void SendEmail(string userEmail, string userName, int lcid, string templateUrl, Message messageType, string role = "", int id = 0)
        {
            ...
            switch (messageType)
            {
                case Message.Reject:
                    if (role == "CUS")
                    {
                        url = templateUrl + "?&l=" + Utility.GetLanguage(lcid) + "&prid=" + ProjectID + "&cid=" + PosterID +"&bid=" + BidderID;
                    }
                    if (role == "PRO")
                    {
                        url = templateUrl + "?&l=" + Utility.GetLanguage(lcid) + "&prid=" + ProjectID + "&pid=" + PosterID + "&bid=" + BidderID;
                    }
                    break;
            }
